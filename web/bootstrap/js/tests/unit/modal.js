$(function () {
  ...
  QUnit.test('should insert into dom when show method is called', function (assert) {
    ...
    $('<div id="modal-servlet"/>')
      .on('shown.bs.modal', function () {
        assert.notEqual($('#modal-servlet').length, 0, 'modal inserted into dom')
        done()
      })
      .bootstrapModal('show')
  })
  ...
  QUnit.test('should hide modal when hide is called', function (assert) {
    ...
    $('<div id="modal-servlet"/>')
      .on('shown.bs.modal', function () {
        assert.ok($('#modal-servlet').is(':visible'), 'modal visible')
        assert.notEqual($('#modal-servlet').length, 0, 'modal inserted into dom')
        $(this).bootstrapModal('hide')
      })
      .on('hidden.bs.modal', function () {
        assert.ok(!$('#modal-servlet').is(':visible'), 'modal hidden')
        done()
      })
      .bootstrapModal('show')
  })
  ...
  QUnit.test('should remove from dom when click [data-dismiss="modal"]', function (assert) {
    ...
    $('<div id="modal-servlet"><span class="close" data-dismiss="modal"/></div>')
      .on('shown.bs.modal', function () {
        assert.ok($('#modal-servlet').is(':visible'), 'modal visible')
        assert.notEqual($('#modal-servlet').length, 0, 'modal inserted into dom')
  ...
  QUnit.test('should allow modal close with "backdrop:false"', function (assert) {
    ...
    $('<div id="modal-servlet" data-backdrop="false"/>')
      .on('shown.bs.modal', function () {
        assert.ok($('#modal-servlet').is(':visible'), 'modal visible')
        $(this).bootstrapModal('hide')
      })
  ...
  QUnit.test('should close modal when clicking outside of modal-content', function (assert) {
    ...
    $('<div id="modal-servlet"><div class="contents"/></div>')
      .on('shown.bs.modal', function () {
        assert.notEqual($('#modal-servlet').length, 0, 'modal inserted into dom')
        $('.contents').trigger('click')
        assert.ok($('#modal-servlet').is(':visible'), 'modal visible')
        $('#modal-servlet').trigger('click')
      })
      .on('hidden.bs.modal', function () {
        assert.ok(!$('#modal-servlet').is(':visible'), 'modal hidden')
        done()
  ...
  QUnit.test('should close modal when escape key is pressed via keydown', function (assert) {
    ...
    var $div = $('<div id="modal-servlet"/>')
    $div
      .on('shown.bs.modal', function () {
        assert.ok($('#modal-servlet').length, 'modal insterted into dom')
        assert.ok($('#modal-servlet').is(':visible'), 'modal visible')
  ...
  QUnit.test('should not close modal when escape key is pressed via keyup', function (assert) {
    ...
    $div
      .on('shown.bs.modal', function () {
        assert.ok($('#modal-servlet').length, 'modal inserted into dom')
        assert.ok($('#modal-servlet').is(':visible'), 'modal visible')
  ...
  QUnit.test('should close reopened modal with [data-dismiss="modal"] click', function (assert) {
    ...
    $('<div id="modal-servlet"><div class="contents"><div id="close" data-dismiss="modal"/></div></div>')
  ...
  QUnit.test('should restore focus to toggling element when modal is hidden after having been opened via data-api', function (assert) {
    ...
    var $toggleBtn = $('<button data-toggle="modal" data-target="#modal-servlet"/>').appendTo('#qunit-fixture')
  ...
  QUnit.test('should not restore focus to toggling element if the associated show event gets prevented', function (assert) {
    ...
    $('<div id="modal-servlet"><div class="contents"><div id="close" data-dismiss="modal"/></div>')
